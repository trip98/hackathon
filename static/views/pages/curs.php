<div class="curs-main">
	<div class="banner">
		<video width="400" height="300" controls="controls" autoplay="true" loop="true" muted="muted">
			<source src="/static/files/lesson.mp4" type='video/mp4; codecs="avc1.42E01E, mp4a.40.2"'>
		</video>
		<div class="label left">
			<h2>Математика в тишине</h2>
			<h3>Цель</h3>
			<p>Подготовка к ЕГЭ</p>
			<h3>Для кого полезно</h3>
			<p>Школьники 9-10 класса</p>
			<div class="button-groups">
				<a href='/lesson?id=1'><button class="clasic">Купить сейчас</button></a>
				<button class="clasic">Перевести урок</button>
			</div>
		</div>
	</div>
	<div class="curs-content">
		<div class="lesson">
			<h3>Дробь</h3>
			<!-- <img src="zxcv.asdfa" alt=""> -->
			<video autoplay="true" loop="true" muted="muted">
				<source src='/static/files/t1_z_teo.mp4' type='video/mp4'>
			</video>
			<a href='/lesson?id=1'><button class="clasic">Купить урок 2990Р</button></a>
		</div>
		<div class="lesson">
			<h3>Умножение</h3>
			<img src="static/img/1.gif" alt="">
			<a href='/lesson?id=1'><button class="clasic">Купить урок 2990Р</button></a>
		</div>
		<div class="lesson">
			<h3>Сложение</h3>
			<img src="static/img/1.gif" alt="">
			<a href='/lesson?id=1'><button class="clasic">Купить урок 2990Р</button></a>
		</div>
		<div class="lesson">
			<h3>Вычитинае</h3>
			<img src="static/img/1.gif" alt="">
			<a href='/lesson?id=1'><button class="clasic">Купить урок 2990Р</button></a>
		</div>
		<div class="lesson">
			<h3>Деление</h3>
			<img src="asd.asdf" alt="">
			<a href='/lesson?id=1'><button class="clasic">Купить урок 2990Р</button></a>
		</div>
		<div class="lesson">
			<h3>Умножение</h3>
			<img src="asd.asdf" alt="">
			<a href='/lesson?id=1'><button class="clasic">Купить урок 2990Р</button></a>
		</div>
		<div class="lesson">
			<h3>Сложение</h3>
			<img src="asd.asdf" alt="">
			<a href='/lesson?id=1'><button class="clasic">Купить урок 2990Р</button></a>
		</div>
		<div class="lesson">
			<h3>Вычитание</h3>
			<img src="asd.asdf" alt="">
			<a href='/lesson?id=1'><button class="clasic">Купить урок 2990Р</button></a>
		</div>
	</div>
</div>